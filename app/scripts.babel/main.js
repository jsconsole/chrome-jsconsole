const js = function() {
  ...
  const _cssParser = function(innerStyle = {}) {
    const css = `color: ${innerStyle.color ||
      style.color}; font-size: ${innerStyle.fontSize ||
      style.fontSize}; font-weight: ${innerStyle.fontWeight ||
      style.fontWeight}; line-height: ${innerStyle.lineHeight ||
      style.lineHeight};`;
    return css;
  };
      ...
      data.forEach(function(singleData, index) {
        ...
        if (singleData !== null && typeof singleData === "object") {
          ...
          singleData.results.forEach(function(result) {
            if (
              result.name.toLowerCase() ===
                loadingValues[index].toLowerCase() ||
              result.name.toLowerCase() ===
                `${loadingValues[index]}.js`.toLowerCase()
            ) {
              singleResult = result;
            }
          });
          if (singleResult && singleResult.latest) {
            _print(
              `Loading... ${singleResult.name} with version ${singleResult.version} from ${singleResult.latest}`
            );
            ...
            _print(`Sorry no exact cdn found for "${loadingValues[index]}"`);
            _print(
              `Did you mean "${loadingValues[
                index
  ...
  const rainbow = function() {
    ...
    console.log("%cJSconsole %s", css, "happy coding good and lovely people");
  };
  const unload = function(force) {
    if (force === true) {
      window.location.reload();
    } else {
      if (
        confirm("Are you sure to unload all the libraries and refresh the page")
      ) {
        window.location.reload();
      }
    }
